...
void ReadMatchedEdge(string MatchedEdgeFile, vector <int> &MatchedEID)
{
    ...
    if(fp == NULL)
        cout  <<  ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.Could not open the file "  << MatchedEdgeFile << endl;
    int eid, index = 0;
...
//    }
    ...
    for(i = 0;  i < size-2; i++)
    {
        ...
        if(DetectEID[i] == DetectEID[i+2] && front > middle && back > middle && DetectEID[i] != 0&&middle < 10)
        {
